...
void Gui::Player::drawPlayerBox(ray_info_t &rayInfo, player_t &player)
{
    if (rayInfo.type == Gui::PLAYER && rayInfo.id == player.n) {
        rayInfo.box.min = (Vector3){player.real_pos.x * Gui::MAP_SCALE -1.0, 0.0, player.real_pos.y * Gui::MAP_SCALE - 1.0};
        rayInfo.box.max = (Vector3){player.real_pos.x * Gui::MAP_SCALE + 1.0, 3.0, player.real_pos.y * Gui::MAP_SCALE + 1.0};
