...
Gui::Graphic::Graphic(std::shared_ptr<GameState> gameState)
    : _gameState(gameState), window(), camera(), skyBox(gameState), object(gameState), island(gameState), player(gameState), menu(gameState), particle(gameState), textBoxs(gameState, window.window_width, window.window_height), egg(gameState)
{
    this->setFps(60);
    this->initLighting();
    this->loop();
