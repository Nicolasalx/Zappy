...
static void draw_player_box(Gui::Graphic *graphic, Gui::player_t &player)
{
    if (graphic->rayInfo.type == Gui::PLAYER && graphic->rayInfo.id == player.n) {
        ...
        DrawModelEx(graphic->model_list[Gui::PLAYER], (Vector3){player.real_pos.x * SCALE, 0.0f, player.real_pos.y * SCALE}, (Vector3){0, 1, 0}, player.real_orientation, (Vector3){3, 3, 3}, GREEN);
    }
}

static void draw_player_model(Gui::Graphic *graphic, Gui::player_t &player)
{
    DrawModelEx(graphic->model_list[Gui::PLAYER], (Vector3){player.real_pos.x * SCALE, 0.0f, player.real_pos.y * SCALE}, (Vector3){0, 1, 0}, player.real_orientation, (Vector3){3, 3, 3}, WHITE);
}
