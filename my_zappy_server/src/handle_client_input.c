    ...
    node_t *current, int i, server_t *server)
{
    ...
    do {
        ...
        GET_DATA(current, egg_t)->pos_y, server->game.team_list[i].name);
    ...
    server_t *server, client_t *client, char *cmd)
{
    ...
    for (int i = 0; i < server->game.team_count; ++i) {
        strcpy(buff, server->game.team_list[i].name);
        strcat(buff, "\n");
        if (strcmp(cmd, buff) == 0) {
            client->player.team = &server->game.team_list[i];
