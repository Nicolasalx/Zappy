...
int calculate_direction(client_t *src, client_t *dest)
{
    ...
    if (abs(dx) > abs(dy)) {
        direction = (dx > 0) ? 3 : 7;  // Right or Left
    } else {
        direction = (dy > 0) ? 1 : 5;  // Up or Down
    }

    if (dx > 0 && dy > 0) direction = 2;
    if (dx > 0 && dy < 0) direction = 4;
    if (dx < 0 && dy < 0) direction = 6;
...
void broadcast_cmd(char *argv, client_t *client, server_t *server)
{
    ...
    for (int i = 0; i < MAX_CLIENT; ++i) {
        ...
        && server->clients[i].player.team && client->player.id != server->clients[i].player.id) {
            int direction = calculate_direction(client, &server->clients[i]);
