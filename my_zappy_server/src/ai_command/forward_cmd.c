...
void forward_cmd(char *argv, client_t *client, server_t *server)
{
    switch (client->player.orientation) {
        case NORTH:
            client->player.pos_y = (client->player.pos_y - 1 + server->world.size_y) % server->world.size_y;
            break;
        case SOUTH:
            client->player.pos_y = (client->player.pos_y + 1) % server->world.size_y;
            break;
        case WEST:
            client->player.pos_x = (client->player.pos_x - 1 + server->world.size_x) % server->world.size_x;
            break;
        case EAST:
